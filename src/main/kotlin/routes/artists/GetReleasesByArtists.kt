...
@Serializable
private data class GetReleasesByArtistResponse(
	val releases: List<Album> = listOf()
)

fun Route.getReleasesByArtist() {
	get("{id}/releases") {
		...
		call.respond(GetReleasesByArtistResponse(
			releases = transaction { artist.albums.map { it.toBaseDTO() } }
		))
	}
}